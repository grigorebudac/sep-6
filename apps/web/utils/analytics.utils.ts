...
const getAverageMovieRatingOverTheYearsOfActor = async (personId: number): Promise<Analytics.AverageRatingOverYears[]> => {
  let avereageRatingOverYears: Analytics.AverageRatingOverYears[] = [];
  const response = await store.dispatch(DiscoverEndpoints.endpoints.getMoviesOfActor.initiate({ personId, page: 1 }))
